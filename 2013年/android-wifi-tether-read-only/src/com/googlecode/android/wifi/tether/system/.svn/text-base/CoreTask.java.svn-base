...
//	}
    ...
    /*
     * This method check if needed kernel-options are available
     */
    public boolean isMSSClampingSupported() {
    	if ((new File("/proc/config.gz")).exists() == false) 
    		return false;
		if (!Configuration.hasKernelFeature("CONFIG_NETFILTER_XT_TARGET_TCPMSS="))
			return false;
		return true;
    }
...
//    		Log.e(TAG, "Unable to update bin/tether with new lan-configuration.");
//    		return writesuccess;
//    	}
